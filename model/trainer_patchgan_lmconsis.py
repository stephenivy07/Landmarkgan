...
from model.net import FSNet_lmarks_1767_patchgan_lmconsis_256, FSNet_lmarks_multiscale_patchgan_lmconsis_256, FSNet_lmarks_mean_lm_id_patchgan_lmconsis_256
...
class Trainer(nn.Module):
    def __init__(self, decoder_num, cfg, face_lm_model):
        ...
        self.model = FSNet_lmarks_1767_patchgan_lmconsis_256(decoder_num, cfg)
        ...
        self.gen_lm_AE_scheduler = get_scheduler(self.gen_lm_AE_opt, cfg)
    ...
    def resume(self, checkpoint_dir, cfg, multigpus=False, index=-1):
        this_model = self.model.module if multigpus else self.model

        last_model_name = get_model_list(checkpoint_dir, "gen", index)
        state_dict = torch.load(last_model_name)
        this_model.gen.load_state_dict(state_dict['gen'])
        # this_model.gen_test.load_state_dict(state_dict['gen_test'])
        iterations = int(last_model_name[-11:-3])

        last_model_name = get_model_list(checkpoint_dir, "dis", index)
        state_dict = torch.load(last_model_name)
        this_model.dis.load_state_dict(state_dict['dis'])

        last_model_name = get_model_list(checkpoint_dir, "lm_encoder", index)
        state_dict = torch.load(last_model_name)
        this_model.lm_encoder.load_state_dict(state_dict['lm_encoder'])

        last_model_name = get_model_list(checkpoint_dir, "lm_decoder", index)
        state_dict = torch.load(last_model_name)
        ...
        if 'is_hm_AE' in list(self.cfg.lm_AE.keys()):
            if self.cfg.lm_AE.is_hm_AE:
                last_model_name = get_model_list(checkpoint_dir, "gaumap_coder", index)
                state_dict = torch.load(last_model_name)
                this_model.gaumap_encoder.load_state_dict(state_dict['gaumap_encoder'])
        ...
        self.gen_lm_AE_opt.load_state_dict(state_dict['gen_lm_AE'])
    ...
    def save(self, snapshot_dir, iterations, multigpus):
        ...
        torch.save({'gen_lm_AE': self.gen_lm_AE_opt.state_dict(),
