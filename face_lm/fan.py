...
class FAN(nn.Module):
    ...
    def __init__(self, nStack=4, is_train=False, nModules=1, nHgDepth=4, num_feats=256, num_classes=68):
        super(FAN, self).__init__()

        self.inplanes = 64
        self.expansion = 2
        self.nStack = nStack
        self.is_train = is_train       #num of how many HourGlassNet stack to the whole FAN Net
        ...
        print('==================>{} keypoints!'.format(self.num_classes))
        ...
        for hg_stack in range(self.nStack):
            ...
            if hg_stack < self.nStack - 1:
                ...
                self.add_module('al' + str(hg_stack), nn.Conv2d(self.num_classes, self.num_feats, kernel_size=1, stride=1, padding=0))

            self.add_module('hm_conv' + str(hg_stack), nn.Conv2d(self.num_classes, self.num_classes, kernel_size=1, bias=False))

        if is_train:
            print('===================>face train mode!')
            self.apply(weights_init('kaiming'))

        # self.hm_conv = nn.Conv2d(68, 68, kernel_size=1, bias=False)
    def forward(self, x):
        ...
        for i in range(self.nStack):
            ...
            # with well-trained model, plot tmp_out, tmp_uhm, tmp_cors
            # plot_heatmap(tmp_out)
            tmp_uhm = self._modules['hm_conv' + str(i)](tmp_out)

            # plot_heatmap(tmp_uhm)

            tmp_hm = dsntnn.flat_softmax(tmp_uhm)

            tmp_cors = dsntnn.dsnt(tmp_hm)


            # TODO delete after debug
            # for iii in range(64):
            #     for jjj in range(64):
            #         if tmp_hm[0][0][iii][jjj] < 0.0001:
            #             tmp_hm[0][0][iii][jjj] = 0.0
            #
            #
            # # plot_heatmap(tmp_uhm)
            #
            # # tmp_hm_fied = dsntnn.flat_softmax(tmp_uhm)
            #
            # tmp_cors_fied = dsntnn.dsnt(tmp_hm)
            #
            #
            # # You need to know how to convert lm to tmp_hm
            # sample = 0
            # print(tmp_cors[sample][0])
            # print(tmp_cors_fied[sample][0])
            #
            # plot_heatmap(tmp_hm, sample, 0)
            #
            # print(tmp_cors[sample][46-33])
            # plot_heatmap(tmp_hm, sample, 46-33)
            #
            # print(tmp_cors[sample][53-33])
            # plot_heatmap(tmp_hm, sample, 53-33)
            #
            # print(tmp_cors[sample][85-33])
            # plot_heatmap(tmp_hm, sample, 85-33)
            #
            # print(tmp_cors[sample][76-33])
            # plot_heatmap(tmp_hm, sample, 76-33)
            #
            # print(tmp_cors[sample][64 - 33])
            # plot_heatmap(tmp_hm, sample, 64 - 33)
            #
            # print(tmp_cors[sample][96 - 33])
            # plot_heatmap(tmp_hm, sample, 96 - 33)



            outputs.append(tmp_hm)
            cors.append(tmp_cors)

        return cors, tmp_uhm, outputs
